...
AnswerGraphCtrl = function($scope, $http, $rootElement, $routeParams, $location, $filter) {
  ...
  render = function(values) {
    ...
    values = _.filter(values, function(d) {
      return d.question.toLowerCase() === $scope.question && d.profil.toLowerCase() === $scope.sample;
    });
    if (values.length === 0) {
      return;
    }
    ...
    x.range([0, w]);
    y.range([h, 0]);
    xAxis = d3.svg.axis().scale(x).tickSize(0).tickPadding(10).tickFormat(dateFormat).ticks(d3.time.months, 2);
