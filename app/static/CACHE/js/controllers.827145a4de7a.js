...
AnswerGraphCtrl = function($scope, $http, $rootElement, $routeParams, $location) {
  var $wrapper, dateFormat, insertLinebreaks, parse, render, svg, update, wrapperHeight, wrapperWidth, x, y;
  ...
  return render = function(values) {
    ...
    svg = d3.select($rootElement.find(".graph")).append("svg:svg").attr("width", w + p[1] + p[3]).attr("height", h + p[0] + p[2]).append("g").attr("transform", "translate(" + p[3] + "," + p[0] + ")");
