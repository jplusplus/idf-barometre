...
AnswerGraphCtrl = function($scope, Answer, $rootElement, $routeParams, $location, $filter) {
  var chart, dateFormat, insertLinebreaks, parse, render, svg, update, wrapper, wrapperHeight, wrapperWidth, x, y;
  ...
  update = function() {
    ...
    return $scope.answers = Answer.query(params, render);
  };
  render = function() {
    ...
    _.each($scope.answers, function(d) {
    ...
    minDate = d3.min($scope.answers, function(d) {
      return d.date;
    });
    maxDate = d3.max($scope.answers, function(d) {
      return d.date;
    });
    minValue = d3.min($scope.answers, function(d) {
      return d.ratio;
    });
    maxValue = d3.max($scope.answers, function(d) {
      return d.ratio;
    });
    ...
    svg = d3.select(chart[0]).append("svg:svg").attr("width", w + p[1] + p[3]).attr("height", h + p[0] + p[2]).append("g").attr("transform", "translate(" + p[3] + "," + p[0] + ")");
    if (!Modernizr.svg) {
      svg.append("svg:path").attr("class", "area bg").attr("fill", "#ee9807").attr("d", area($scope.answers));
    } else {
      svg.append("linearGradient").attr("id", "sequence-gradient").attr("gradientUnits", "userSpaceOnUse").attr("spreadMethod", "repeat").attr("y1", 0).attr("x1", 0).attr("y2", 0).attr("x2", gradientW).selectAll("stop").data([
        {
          offset: "0%",
          color: $filter("colors")($scope.question, "0")
        }, {
          offset: "50%",
          color: $filter("colors")($scope.question, "50")
        }, {
          offset: "51%",
          color: $filter("colors")($scope.question, "51")
        }, {
          offset: "100%",
          color: $filter("colors")($scope.question, "100")
        }
      ...
      svg.append("svg:path").attr("class", "area bg").attr("fill", "url(#sequence-gradient)").attr("d", area($scope.answers));
    }
    svg.selectAll(".data-point").data($scope.answers).enter().append("svg:circle").attr("class", "data-point").attr("cx", function(d) {
      return x(d.date);
    }).attr("cy", function(d) {
      return y(d.ratio);
    }).attr("fill", $filter("colors")($scope.question)).attr("stroke-width", 3).attr("stroke", "#ffffff").attr("r", 5);
    ...
    return wrapper.data("jsp").reinitialise();
  };
  return update();
};
